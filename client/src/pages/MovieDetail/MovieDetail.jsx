      ...
      try {
        ...
        if (movieData?.categories?.length > 0) {
          try {
            ...
            if (
              typeof movieData.categories[0] === "object" &&
              movieData.categories[0]?._id
            ) {
            ...
            try {
              const relatedResponse = await movieService.getMoviesByCategory(
                categoryId
              );
        ...
        try {
          const commentsData = await commentService.getCommentsByMovieId(
            movieData._id
          );
          if (commentsData) {
            const formattedComments = Array.isArray(commentsData)
              ? commentsData
                  .filter((comment) => comment.user !== null) // Lọc bỏ comment có user null
                  .map((comment) => {
                    const userObj =
  ...
  const handleRatingSubmit = async (ratingValue) => {
    ...
    try {
      ...
      if (error.response && error.response.data) {
        toast.error(
          error.response.data.message || "Có lỗi xảy ra khi đánh giá phim"
        );
      } else {
